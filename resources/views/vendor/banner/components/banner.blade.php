<div @class([
    ...
    @if ($banner->isVisible())
        ...
        @endphp
        <div x-title="banner-component" x-cloak x-show="show" x-data="{
            show: true,
            ...
            <div class="col-span-11 flex items-center">
                <div>
                    @if ($banner->icon)
                        <x-filament::icon alias="banner::close" :icon="$banner->icon"
                            style="color: {{ $banner->icon_color ?? '#FFFFFF' }}"
                            class="h-6 w-6 mr-2 text-zinc-500 dark:text-zinc-400 text-white" />
                    @endif
                </div>
                <div>
                    {!! $banner->content !!}
            ...
            <div class="col-span-1 flex justify-end">
                @if ($banner->can_be_closed_by_user)
                    <x-filament::icon x-on:click="close" alias="banner::close" icon="heroicon-m-x-mark"
                        class="h-6 w-6 text-zinc-500 dark:text-zinc-400 text-white cursor-pointer hover:opacity-75" />
                @endif
            </div>
        </div>
    @endif
