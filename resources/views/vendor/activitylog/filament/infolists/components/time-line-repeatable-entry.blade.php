...
<x-dynamic-component :component="$getEntryWrapperView()" :entry="$entry">
    <div
        ...
        @if (count($childComponentContainers = $getChildComponentContainers()))
            <ol class="relative border-zinc-200 border-s dark:border-zinc-700">
                <x-filament::grid :default="$getGridColumns('default')" :sm="$getGridColumns('sm')" :md="$getGridColumns('md')" :lg="$getGridColumns('lg')"
                    ...
                    @foreach ($childComponentContainers as $container)
                        <li @class([
                            ...
                            'rounded-xl bg-white p-4 shadow-sm ring-1 ring-zinc-950/5 dark:bg-white/5 dark:ring-white/10' => $isContained,
                        ])>
                            {{ $container }}
                        </li>
                    @endforeach
